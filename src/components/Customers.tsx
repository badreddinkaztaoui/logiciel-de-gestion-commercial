    ...
    <div className="h-full flex flex-col p-6 space-y-6 overflow-y-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
        <div>
          ...
          <p className="text-gray-600 text-sm">Gérez vos clients pour les factures et devis</p>
        </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          >
            <RefreshCw className={`w-4 h-4 ${syncStatus === 'syncing' ? 'animate-spin text-blue-600' : 'text-gray-500'}`} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-gray-500">WooCommerce</p>
              <p className="text-2xl font-semibold text-gray-900">{stats.withWooCommerceId}</p>
            </div>
          </div>
        </div>
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        ...
                        {customer.woocommerce_id && (
                          <div className="text-xs text-purple-600">WooCommerce ID: {customer.woocommerce_id}</div>
                        )}
                      </div>
                    </div>
                  </td>
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full">
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    <p><span className="text-gray-500">Nom complet:</span> {selectedCustomer.first_name} {selectedCustomer.last_name}</p>
                    <p><span className="text-gray-500">Email:</span> {selectedCustomer.email}</p>
                    {selectedCustomer.phone && (
                      <p><span className="text-gray-500">Téléphone:</span> {selectedCustomer.phone}</p>
                    )}
                    {selectedCustomer.woocommerce_id && (
                      <p className="flex items-center text-purple-600">
                        <RefreshCw className="w-3 h-3 mr-1" />
                        Synchronisé avec WooCommerce (ID: {selectedCustomer.woocommerce_id})
                      </p>
                    )}
                  </div>
                </div>
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    {selectedCustomer.company ? (
                      <p><span className="text-gray-500">Entreprise:</span> {selectedCustomer.company}</p>
                    ) : (
                      <p className="text-gray-400">Aucune entreprise renseignée</p>
                    )}
                    {selectedCustomer.ice && (
                      <p><span className="text-gray-500">ICE:</span> {selectedCustomer.ice}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="mt-6 text-sm text-gray-500">
                {selectedCustomer.created_at && <p>Créé le {formatDate(selectedCustomer.created_at)}</p>}
                {selectedCustomer.updated_at && <p>Dernière mise à jour le {formatDate(selectedCustomer.updated_at)}</p>}
              </div>
